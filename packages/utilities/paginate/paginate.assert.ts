...
assert(
  paginate(testData, "id", undefined, 3).data.length === 3,
  "Should return 3 items",
);

// TEST: Check if the function returns the correct number of items from the end for a negative take
assert(
  paginate(testData, "id", undefined, -2).data.length === 2,
  "Should return 2 items from the end",
);

// TEST: Validate the first and last elements in the paginated result
const result = paginate(testData, "id", undefined, 2);
assert(
  result.first === null && result.last === testData[2].id,
  "Incorrect first or last element",
);

// TEST: Check cursor-based pagination
assert(
  paginate(testData, "id", "2", 2).data[0].id === "3",
  "Cursor not correctly setting start position",
);

// TEST: Check if 'hasMore' flag is correctly calculated
assert(
  paginate(testData, "id", undefined, 5).hasMore === false,
  "hasMore should be false when all items are taken",
);
...
assert(
  paginate(testData, "id", "1", 2).data.length === 2,
  "Boundary check failed at start of array",
);
assert(
  paginate(testData, "id", "5", -2).data.length === 2,
  "Boundary check failed at end of array",
);
...
assert(
  JSON.stringify(actualDataForPositiveTake) ===
    JSON.stringify(expectedDataForPositiveTake),
  "Data mismatch for positive take",
);

const expectedDataForNegativeTake = testData.slice(-2);
const actualDataForNegativeTake = paginate(testData, "id", undefined, -2).data;
assert(
  JSON.stringify(actualDataForNegativeTake) ===
    JSON.stringify(expectedDataForNegativeTake),
  "Data mismatch for negative take",
);
...
assert(
  JSON.stringify(actualDataWithCursor) ===
    JSON.stringify(expectedDataWithCursor),
  "Data mismatch with cursor",
);
