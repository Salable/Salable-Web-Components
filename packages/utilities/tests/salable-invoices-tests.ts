...
export async function salableInvoiceTests(
  page: Page,
  invoiceRepository: InvoiceRepository,
) {
...
async function testInvoicesRows(rows: Invoice[], tableRows: Locator) {
  for (const [i, rowData] of rows.entries()) {
    ...
    expect(await invoiceLink.getAttribute("href")).toBe(
      rowData.hosted_invoice_url,
    );
    await expect(invoiceLink).toContainText(rowData.number);

    await expect(cells.nth(1).locator("salable-date")).toContainText(
      "04 April 1968",
    );
    await expect(cells.nth(2).locator("salable-status")).toContainText(
      rowData.status,
      { ignoreCase: true },
    );

    const pdfDownloadLink = cells.nth(3).locator("a");
    expect(await pdfDownloadLink.getAttribute("href")).toBe(
      rowData.invoice_pdf,
    );
