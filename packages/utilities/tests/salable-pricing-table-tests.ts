...
export async function salablePricingTableTests(page: Page) {
    const intervalToggle = page.locator('#interval-toggle');
    await expect(intervalToggle).toBeVisible();
    await expect(intervalToggle).not.toBeChecked();
    await testMonthlyLicensedPricingTable(page);
    await intervalToggle.check();
    await testYearlyLicensedPricingTable(page);
}

async function testMonthlyLicensedPricingTable(page: Page) {
    const firstCard = page.getByTestId('pricing-table-card-0');
    ...
    const secondCard = page.getByTestId('pricing-table-card-1');
...
async function testYearlyLicensedPricingTable(page: Page) {
    const firstCard = page.getByTestId('pricing-table-card-0');
    await expect(firstCard).toBeVisible();
