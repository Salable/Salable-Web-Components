...
import { defaultCurrency, pricingTableMock, pricingTablePlanMock } from '../../../../utilities/mock-data/pricing-table.mock';
import { expect } from '@playwright/test';

test.describe('salable-pricing-table Stencil E2E Tests', () => {
  const mockSessionToken = 'test_xxxxxx';
    ...
    test('Displays a product pricing table and toggles between monthly/yearly intervals', async ({ page }) => {
      ...
      await page.setContent(`
          <salable-pricing-table
            session-token="${mockSessionToken}"
  ...
  test.describe('Fetch Error Cases', () => {
    ...
    test('Displays an error message if checkout fetch fails', async ({ page }) => {
      ...
      const waitCheckoutFetchResponse = page.waitForResponse(res => res.url().includes(`/checkoutlink`) && res.request().method() === 'GET' && res.status() === 400);
    ...
    test('Displays an error message if create license fetch fails', async ({ page }) => {
      ...
      const waitLicensesFetchResponse = page.waitForResponse(async res => {
        return res.url().includes(`/licenses`) && res.request().method() === 'POST' && res.status() === 400;
      });
